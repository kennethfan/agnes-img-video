...
async function handleRestore(rawFile: File) {
  ...
  if (file.name.endsWith('.sql')) {
    ...
    ElMessage.warning('请选择 .db、.sqlite 或 .sql 文件')
    return false
  }

  uploading.value = true
  try {
    const msg = file.name.endsWith('.sql') ? 'SQL 执行成功' : '数据库恢复成功'
...
<template>
  <div class="db-manage">
    <h3>备份恢复</h3>
    <p class="desc">history.db 包含所有历史记录、作品库、故事板数据。支持 .db/.sql 格式。</p>
    ...
    <div class="cards">
      ...
      <div class="card">
        ...
        <div class="card-desc">上传 .db / .sqlite / .sql 文件。<br>操作不可恢复，建议先导出备份。</div>
        <el-upload
